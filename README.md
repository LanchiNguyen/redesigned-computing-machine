...
- **`v2/`** — includes two functional concept prototypes, both honestly labeled (illustrative data, no user studies claimed):
  - **Tenet** (`v2/tenet.html`, № 01) — a behavioral trading-guardrails system: self-authored rules become reversible friction at order-send inside two fictional brokers (Dart mobile, Meridian desktop) with one Companion Record. Assets in `images/tenet/` (native compositions rendered from the prototype's screen designs).
  - **Morsel** (`v2/morsel.html`, № 02) — photo-first food discovery: three prototype versions, 30 designed states, an allergy-aware dietary model, wireflow/explorations/DS/testing-plan artifacts (live in `v2/morsel-docs/`). Assets in `images/morsel/`.
